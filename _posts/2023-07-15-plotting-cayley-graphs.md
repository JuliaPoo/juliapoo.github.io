...
This explains the donut shape of $C_{10} \times C_{20}$. In the Cayley Graph of $A \times B$, all copies of the element $a \in A$ are strung together, and similarly for all elements of $b \in B$. Try [plotting other direct products](https://juliapoo.github.io/Cayley-Graph-Plotting/) and play with the edge lengths and see if you can spot this pattern.
...
Semi-direct products introduce an additional construct to direct products, which we'll call a _twist_ and denote with the symbol $\psi$. We'll denote a semi-direct product of groups $A$ and $B$ with the twist $\psi$ as the group $G = A \ltimes_\psi B$. What $\psi$ is precisely will be described later. First, I'll illustrate the concept of a _twist_ via Cayley Graphs.

<!--TODO: cool diagram-->

Now we try to derive semi-direct products as a generalisation of direct products from scratch. First, some terminology. I represent an element of $G = A \ltimes_\psi B$ similarly to direct products: as a tuple $(a,b)$ where $a \in A$ and $b \in B$. I'll refer to $a$ as the $A$-component of $(a,b)$ (and similarly for the $B$-component). As we attempt to generalise direct products into semi-direct products, we attempt to generalise the group operation of $G$. 
...
Similarly, let's also embed the group operation of $B$ into $G$ by requiring that $(a,b) \cdot (e,b') = (a, bb')$, i.e., the right multiplication with elements with no $B$-component is equivalent to the group operation of $B$. This is my interpretation of the "semi" in "semi-direct product":
...
We should also analyse how $f_b$ of different $b \in B$ interact:
...
And our semi-direct product reduces into a direct product. Hence direct products are simply a subset of semi-direct products.
